...
const REFERENCE_MODE = new URLSearchParams(location.search).has('reference');
...
function verifyAll({ quiet = false } = {}) {
  ...
  if (!quiet) {
    if (allPass) {
      showBanner('resonant', 'The commission is filled', 'Every performance rang true.', levelIndex < LEVELS.length - 1);
    } else {
...
function showBanner(kind, title, detail, showNext) {
...
function drawWord(x, y, word, center = true) {
  const shown = [...word].slice(0, 6);
  const width = Math.max(shown.length, 1) * 14 + 8;
  const left = center ? x - width / 2 : x;
  let s = `<rect class="word-pill-box" x="${left}" y="${y - 12}" width="${width}" height="24" rx="4"></rect>`;
  for (let line = -4; line <= 4; line += 2) {
    s += `<line class="word-staff" x1="${left + 3}" y1="${y + line}" x2="${left + width - 3}" y2="${y + line}"></line>`;
  }
  if (!word) {
    s += `<text class="word-rest" x="${left + width / 2}" y="${y + 5}">𝄽</text>`;
    s += `<text class="note-letter" x="${left + width / 2}" y="${y + 10}">∅</text>`;
  } else {
    shown.forEach((note, i) => {
      const nx = left + 11 + i * 14;
      const ny = y + ({ A: 4, B: 2, C: 0, D: -2 }[note] ?? 0);
      s += `<line class="note-stem" x1="${nx + 3.4}" y1="${ny}" x2="${nx + 3.4}" y2="${ny - 8}"></line>`;
      s += `<ellipse class="note-head" cx="${nx}" cy="${ny}" rx="4.3" ry="3" ` +
        `transform="rotate(-18 ${nx} ${ny})" fill="var(--note-${note})"></ellipse>`;
      s += `<text class="note-letter" x="${nx}" y="${y + 10}">${note}</text>`;
    });
    if (word.length > 6) s += `<text class="word-more" x="${left + width + 2}" y="${y + 3}">+${word.length - 6}</text>`;
  }
  return s;
}
...
function renderBoard() {
  ...
  for (const part of m.parts) {
    ...
    if (part.kind === 'quill') {
      ...
      if (seed !== undefined) s += `<g class="goal-word" data-word="${seed}">${drawWord(cx, cy - 34, seed)}</g>`;
    }
    if (part.kind === 'resonator') {
      const want = kase.targets[part.id];
      if (want !== undefined) s += `<g class="goal-word" data-word="${want}">${drawWord(cx, cy - 34, want)}</g>`;
  ...
  if (run) {
    for (const [motionIndex, item] of run.transit.entries()) {
      ...
      if (discreteFrames) {
        ...
        s += `<g class="transit-word">${drawWord(pos.x, pos.y, item.word)}</g>`;
      ...
      for (const [port, words] of Object.entries(ports)) {
        words.forEach((word, i) => {
          ...
          const routeClass = part.kind === 'coupling'
            ...
            : '';
          s += `<g class="${routeClass}">${drawWord(pos.x - 34, pos.y - i * 16, word)}</g>`;
        });
      }
    }
    for (const h of run.holds) {
      const part = byId(m, h.part);
      s += drawWord(part.x * CELL + CELL / 2, part.y * CELL + CELL / 2 - 34, h.word);
    }
  }
...
function renderAssignment() {
  ...
  $('assignment').innerHTML =
    ...
    `${REFERENCE_MODE ? ' · <span class="ref-mode">REFERENCE MODE — nothing saves</span>' : ''}</small>` +
...
function renderDeck() {
  ...
  $('deck-reference').hidden = deckHidden || walkIndex !== p.walkthrough.length - 1;
...
function renderNav() {
  ...
  $('level-nav').innerHTML = groups.map((g) =>
    ...
    g.items.map(({ p, i }) =>
      `<button class="level-dot${i === levelIndex ? ' current' : ''}${save.solved.includes(p.id) ? ' solved' : ''}" ` +
      `data-level="${i}" title="${p.title}">${i + 1}</button>`
    ).join('') +
    '</span>'
  ).join('');
}
...
$('level-nav').addEventListener('click', (e) => {
  ...
  if (dot) loadLevel(Number(dot.dataset.level));
});

// resume at the saved position when its level still exists; otherwise at the
// first unsolved level, and at the finale when everything is solved
{
  const fromId = LEVELS.findIndex((p) => p.id === save.levelId);
  const firstUnsolved = LEVELS.findIndex((p) => !save.solved.includes(p.id));
  loadLevel(fromId >= 0 ? fromId : firstUnsolved >= 0 ? firstUnsolved : LEVELS.length - 1);
}
